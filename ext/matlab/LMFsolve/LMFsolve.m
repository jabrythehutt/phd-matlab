...
%                                   = 1 otherwise,
%                                     where JJ = J.'*J
% 'CustJac'      {[]}       custJac(x) should return a Jacobian based on x.
...
% 2013-04-23    v 1.3 * Modified by Daniel Jabry djabry@gmail.com to
%                       include option of custom Jacobian calculation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if nargin==1 && strcmpi('default',varargin(1))
   ...
   xf.CustJac  = [];        %   Custom Jacobian set to empty for finite dif
   return
...
elseif isstruct(varargin{1}) % Options=LMFsolve(Options,'Name','Value',...)
    ...
    for i=2:2:nargin-1
        ...
        elseif strncmp(name,'c',1), xf.CustJac  = value;  
...
elseif ischar(varargin{1})   % check for Options=LMFSOLVE('Name',Value,...)
   Pnames=char('display','funtol','xtol','maxiter','scaled','custjac');
...
J = calcJac(FUN,r,x,epsx,options);
%~~~~~~~~~~~~~~~~~~~~~~~
nfJ = 2;
A = J.'*J;                    % System matrix
v = J.'*r;
...
function J = calcJac(FUN,r,x,epsx,options)
custJac = options.CustJac;
if isempty(custJac)
