function [ prof,error_prof,min_prof,max_prof,uMap ] = extractMeanProfile( dataDir,dimx,lims )
...
for i=1:length(allVars)
    ...
    function [concVec,lblunit] = convertUnits(vec1,unitString)
            ...
            if isKey(convMap,foundUnit) 
                convVal2 = convMap(foundUnit);
            
            
            
                concVec = vec1*convVal1*convVal2;
                lblunit = unitMap(foundUnit);
                
            else
                
                disp(['Failed to find unit for unit string: ',unitString]);
