...
for atmIx = 1:length(atmToTest)
    ...
    for obsIx =  1:length(obsAlts)
        ...
        if (min(wn)~=min(result.wn))||(max(wn)~=max(result.wn))
            %Interpolate results to new grid if they're not matching
            disp('Interpolating results to new grid');
            result.rad = interp1(result.wn,result.rad,wn);
            result.args{4} = lblArgs{4};
            
            result.k(result.k>1|result.k<-1)=0;
            
            result.k = interp2(1:length(result.sa),result.wn,result.k,1:length(result.sa),wn,'linear');
            %result.se = sparse(interp2(result.wn,result.wn,result.se,wn,wn),'linear');

            result.wn = wn;
            
            if isfield(result,'lowNoise')
                result = rmfield(result,'lowNoise');
            
            end
            
            if isfield(result,'hightNoise')
                result = rmfield(result,'highNoise');
            end
