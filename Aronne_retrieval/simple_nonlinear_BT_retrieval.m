function [xhat_final, convergence_met, iter, xhat, G, A, K, Fxhat] = ...
    simple_nonlinear_BT_retrieval(fullwn,prior, prior_F, fullSe, ...
...
%Convert all inputs to be in terms of BT

function k = convertFullK(kIn,wn,R,fullx)

    
    k=convertJacobian(kIn,wn,R,fullx,0,2);

end


prior_F.K=convertFullK(prior_F.K,fullwn,prior_F.Fxhat,prior.x0);
prior_F.Fxhat = convertToBT(prior_F.Fxhat,fullwn);
fullSe =convertSE(fullSe,fullwn,fully,0,1);
fully = convertToBT(fully,fullwn);
...
while ~convergence_met && (iter <= max_iteration_count)
    ...
    if iter > 1
        ...
        if lblrtm_success
            ...
            newFullK=convertFullK(newFullK,fullwn,newy,fullx);
            newy = convertToBT(newy,fullwn);
    ...
    inv_hatS = K{iter}' * inv_Se * K{iter} + inv_Sa;
    hatS = inv(inv_hatS);
    G{iter} = hatS * K{iter}' * inv_Se;
    A{iter} = G{iter} * K{iter};

    ylocal = y - Fxhat{iter} + K{iter}*(xhat{iter} - xa);
...
%Convert k and Fxhat back to be in terms of Radiance

for i=1:length(Fxhat)
    
    rad = Fxhat{i};
    rad = convertToRad(rad,fullwn);
    Fxhat{i}=rad;
    
    jac = K{i};
    jac = convertJacobian(jac,fullwn,rad,xhat{i},2,0);
    K{i}=jac;
...
xhat_final = prior.x0;
xhat_final(state_mask) = xhat{end};
